...
def infer_model_dims(sample: dict[str, Any], layer_names: tuple[str, ...]) -> tuple[int, int]:
    ...
    feature_dim = (
        ...
        if sample.get("target_features") is not None
        else 0
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--head_type",
        choices=["conv", "attention", "kv_cache_attention", "wan_dflash_attention"],
        default="attention",
    )
    ...
    parser.add_argument("--training_mode", choices=["one_step", "unrolled"], default="one_step")
    ...
    if args.init_draft_head_checkpoint_path:
        ...
        if args.head_type not in ("attention", "wan_dflash_attention"):
            raise ValueError("--init_target_blocks is only supported for --head_type attention/wan_dflash_attention")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.parallel_strategy == "fsdp" and is_distributed:
            ...
            epoch_path = run_dir / f"epoch_{epoch:04d}.pt"
            latest_path = run_dir / "latest.pt"
            saved_epoch = rank0_save_with_state_dict(
                model=model,
                unwrapped_model=raw_model,
                path=epoch_path,
                save_fn=save_draft_head_checkpoint,
                save_kwargs={"layer_names": layer_names, "num_blocks": args.num_blocks, "metadata": epoch_metadata},
            )
    ...
    if args.parallel_strategy == "fsdp" and is_distributed:
        ...
        saved_output_path = save_draft_head_checkpoint(
            raw_model,
            output_path,
            layer_names=layer_names,
            num_blocks=args.num_blocks,
