...
SELF_CONDITIONING_LOOKAHEAD_CHUNKS="${SELF_CONDITIONING_LOOKAHEAD_CHUNKS:-2}"
SELF_CONDITIONING_ANCHOR_POLICY="${SELF_CONDITIONING_ANCHOR_POLICY:-random_valid}"
SELF_CONDITIONING_FIXED_ANCHOR_INDEX="${SELF_CONDITIONING_FIXED_ANCHOR_INDEX:-0}"
SELF_CONDITIONING_MIX_RATIO="${SELF_CONDITIONING_MIX_RATIO:-0.25}"
SELF_CONDITIONING_CURRENT_NOISE_MODE="${SELF_CONDITIONING_CURRENT_NOISE_MODE:-teacher_cached}"
SELF_CONDITIONING_CONSISTENCY_WEIGHT="${SELF_CONDITIONING_CONSISTENCY_WEIGHT:-0.0}"
SELF_CONDITIONING_LOSS_ON="${SELF_CONDITIONING_LOSS_ON:-all_predicted_chunks}"
SELF_CONDITIONING_INTERMEDIATE_LOSS_WEIGHT="${SELF_CONDITIONING_INTERMEDIATE_LOSS_WEIGHT:-0.5}"
...
if [[ "$HEAD_TYPE" == "kv_cache_attention" || "$HEAD_TYPE" == "ar_bidirectional" || "$HEAD_TYPE" == "causal_wan_ar" ]] && [[ -z "$INIT_TARGET_BLOCKS_WAS_SET" ]]; then
  INIT_TARGET_BLOCKS=""
fi
...
if [[ "$TRAINING_MODE" == "teacher_trajectory" ]]; then
  ...
  if [[ "$TEACHER_TRAJECTORY_OBJECTIVE" == "stop_gradient_self_conditioning_flow" ]]; then
    FULL_CONFIG_TAG="${FULL_CONFIG_TAG}_sck${SELF_CONDITIONING_LOOKAHEAD_CHUNKS}_scmix$(tag_slug "$SELF_CONDITIONING_MIX_RATIO")_sccur$(tag_slug "$SELF_CONDITIONING_CURRENT_NOISE_MODE")_scap$(tag_slug "$SELF_CONDITIONING_ANCHOR_POLICY")_scloss$(tag_slug "$SELF_CONDITIONING_LOSS_ON")_sccw$(tag_slug "$SELF_CONDITIONING_CONSISTENCY_WEIGHT")"
  fi
fi
...
UNROLL_ARGS+=(--self_conditioning_lookahead_chunks "$SELF_CONDITIONING_LOOKAHEAD_CHUNKS")
UNROLL_ARGS+=(--self_conditioning_anchor_policy "$SELF_CONDITIONING_ANCHOR_POLICY")
UNROLL_ARGS+=(--self_conditioning_fixed_anchor_index "$SELF_CONDITIONING_FIXED_ANCHOR_INDEX")
UNROLL_ARGS+=(--self_conditioning_mix_ratio "$SELF_CONDITIONING_MIX_RATIO")
UNROLL_ARGS+=(--self_conditioning_current_noise_mode "$SELF_CONDITIONING_CURRENT_NOISE_MODE")
UNROLL_ARGS+=(--self_conditioning_consistency_weight "$SELF_CONDITIONING_CONSISTENCY_WEIGHT")
UNROLL_ARGS+=(--self_conditioning_loss_on "$SELF_CONDITIONING_LOSS_ON")
UNROLL_ARGS+=(--self_conditioning_intermediate_loss_weight "$SELF_CONDITIONING_INTERMEDIATE_LOSS_WEIGHT")
if [[ -n "$UNROLL_STEP_WEIGHTS" ]]; then
  # shellcheck disable=SC2206
...
elif [[ "$HEAD_TYPE" == "causal_wan_ar" && -n "$INIT_TARGET_CHECKPOINT_PATH" ]]; then
  TARGET_INIT_ARGS+=(--init_target_checkpoint_path "$INIT_TARGET_CHECKPOINT_PATH")
  TARGET_INIT_ARGS+=(--init_target_model_name "$INIT_TARGET_MODEL_NAME")
fi
...
log "Training: mode=$TRAINING_MODE teacher_step_mode=$TEACHER_TRAJECTORY_STEP_MODE teacher_objective=$TEACHER_TRAJECTORY_OBJECTIVE prefix_loss_weight=$TEACHER_TRAJECTORY_PREFIX_LOSS_WEIGHT incremental_kv_loss_weight=$TEACHER_TRAJECTORY_INCREMENTAL_KV_LOSS_WEIGHT unroll_noise=$UNROLL_NOISE_MODE weights=${UNROLL_STEP_WEIGHTS:-auto} incremental_kv_consistency=$INCREMENTAL_KV_CONSISTENCY_WEIGHT context_noise=$INCREMENTAL_KV_CONTEXT_NOISE self_conditioning_lookahead=$SELF_CONDITIONING_LOOKAHEAD_CHUNKS self_conditioning_mix=$SELF_CONDITIONING_MIX_RATIO self_conditioning_current_noise=$SELF_CONDITIONING_CURRENT_NOISE_MODE self_conditioning_anchor=$SELF_CONDITIONING_ANCHOR_POLICY self_conditioning_consistency=$SELF_CONDITIONING_CONSISTENCY_WEIGHT self_conditioning_loss_on=$SELF_CONDITIONING_LOSS_ON"
