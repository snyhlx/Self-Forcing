...
if [[ "$TRAINING_MODE" == "teacher_trajectory" ]]; then
  ...
  if [[ "$TEACHER_TRAJECTORY_OBJECTIVE" == "stop_gradient_self_conditioning_flow" ]]; then
    FULL_CONFIG_TAG="${FULL_CONFIG_TAG}_sck${SELF_CONDITIONING_LOOKAHEAD_CHUNKS}_scmix$(tag_slug "$SELF_CONDITIONING_MIX_RATIO")_scap$(tag_slug "$SELF_CONDITIONING_ANCHOR_POLICY")_scloss$(tag_slug "$SELF_CONDITIONING_LOSS_ON")_sccw$(tag_slug "$SELF_CONDITIONING_CONSISTENCY_WEIGHT")"
  fi
fi
