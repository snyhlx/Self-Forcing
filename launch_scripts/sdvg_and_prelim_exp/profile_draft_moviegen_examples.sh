#!/usr/bin/env bash
# Profile pure drafter generation on a MovieGenVideoBench prompt subset.
set -euo pipefail
...
SEED="${SEED:-42}"
RUN_TAG="${RUN_TAG:-draft_only_blocks${NUM_BLOCKS}_start${START_INDEX}_max${MAX_PROMPTS}_$(date +%Y%m%d_%H%M%S)}"
...
log "Profiling draft_only MovieGen subset"
...
log "Outputs saved under: $OUTPUT_DIR"
